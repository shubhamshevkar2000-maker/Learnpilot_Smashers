          ...
          {!hasPlan ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/40 pb-3">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-muted-foreground">
                    YOUR ROADMAP ({modulesList.length} Phases)
                  </span>
                  <span className="text-[11px] text-muted-foreground font-medium">
                    • {completedModulesCount} of {modulesList.length} phases completed
                  </span>
                </div>
              ...
              <div className="space-y-8">
                {profile && modulesList.length > 0 && generateSchedule(profile, modulesList).days.map((day) => {
                  const completedMins = day.activities.reduce((sum, act) => sum + (act.is_completed ? (act.estimated_minutes || 20) : 0), 0)
                  const isDayComplete = day.activities.every(a => a.is_completed)
                  const hasIncomplete = day.activities.some(a => !a.is_completed)
                  
                  // Determine if this is "Today"
                  // "Today" is the first day that has an incomplete activity.
                  const schedule = generateSchedule(profile, modulesList)
                  const todayDayNumber = schedule.days.find(d => d.activities.some(a => !a.is_completed))?.dayNumber || schedule.days[schedule.days.length - 1].dayNumber
                    ...
                    <div key={`day-${day.dayNumber}`} className="group border-b border-border/40 pb-6 space-y-4 transition-colors">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{completedMins} / {day.totalMinutes} min</span>
                          </div>
                          <span>•</span>
                          <span className="font-medium">
                            {day.activities.filter(a => a.is_completed).length} / {day.activities.length} tasks
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2.5">
                        {day.activities.map((act) => (
                          <div
                            key={act.id}
                            className={`flex items-center justify-between rounded-xl border ${act.is_completed ? 'border-emerald-500/30 bg-emerald-500/5' : (isToday ? 'border-primary/30 bg-primary/5' : 'border-border/30 bg-card/30')} px-4 py-3 text-sm transition-all`}
                          >
